...
class Trainer():
    def __init__(self, model=None,
                 epoch=100, batch_size=40, ckpt_dir="./temp/",
    ...
    def train(self, edges, train_evaluator, dev_edges, dev_evaluator,test_edges, test_evaluator, checkpoint_dir):
        ...
        for epoch in range(1, self.train_epoch + 1):
            ...
            for batch_ in range(batch_per_epoch):
                ...
                if len(choices)==batch_size:
                    ...
                    #pres = batch_logits.argmax(dim=-1)
                    ...
                    batch_logits = batch_logits.squeeze(dim=-1)

                    #batch_labels = batch_labels.reshape(-1,2)
                    #batch_logits = batch_logits.reshape(32,-1)
            ...
            accu_train.append(trainmetrics[3])
            recall_train.append(trainmetrics[4])

            f1_dev.append(devmetrics[0])
            auc_dev.append(devmetrics[1])
            aupr_dev.append(devmetrics[2])
            accu_dev.append(devmetrics[3])
            recall_dev.append(devmetrics[4])

            f1_test.append(testmetrics[0])
            auc_test.append(testmetrics[1])
            aupr_test.append(testmetrics[2])
            accu_test.append(testmetrics[3])
            recall_test.append(testmetrics[4])
            ...
            np.save(checkpoint_dir + 'accu_train.npy', accu_train)
            np.save(checkpoint_dir + 'recall_train.npy', recall_train)
            ...
            np.save(checkpoint_dir + 'accu_dev.npy', accu_dev)
            np.save(checkpoint_dir + 'recall_dev.npy', recall_dev)
            ...
            np.save(checkpoint_dir + 'accu_test.npy', accu_test)
            np.save(checkpoint_dir + 'recall_test.npy', recall_test)
            ...
            record_dict['train_accu'].append(trainmetrics[3])
            record_dict['train_recall'].append(trainmetrics[4])
            record_dict['dev_f1'].append(devmetrics[0])
            record_dict['dev_auc'].append(devmetrics[1])
            record_dict['dev_aupr'].append(devmetrics[2])
            record_dict['dev_accu'].append(devmetrics[3])
            record_dict['dev_recall'].append(devmetrics[4])
        ...
        return record_dict,loss_train,f1_train,auc_train,aupr_train,recall_train,f1_dev,auc_dev,aupr_dev,accu_train,accu_dev,recall_dev
